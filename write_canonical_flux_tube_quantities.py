...
def main(args):
    ...
    assert (args.task == 'write_to_vtk' or
            args.task == 'write_to_npz' or
            args.task == 'calculate_helicity'), 'Invalid task: ' + args.task

    now = datetime.now().strftime("%Y-%m-%d-%H-%M")
    out_dir = '../output/write_to_vtk/' + now
    try:
        os.makedirs(out_dir)
    except:
        pass
    ...
    A_vacuum_theta = (args.bias_field_magnitude/2. *
                      np.sqrt(mesh_wo_edges[0]**2. +
                              mesh_wo_edges[1]**2.))
    A_vacuum_x = A_vacuum_theta * np.cos(np.arctan2(mesh_wo_edges[1], mesh_wo_edges[0]))
    A_vacuum_y = A_vacuum_theta * np.sin(np.arctan2(mesh_wo_edges[1], mesh_wo_edges[0]))

    for time_point in xrange(len(bx_interpolators)):

        print time_point
        mesh = np.meshgrid(np.linspace(x_min, x_max,
                                       np.ceil((x_max-x_min)/args.spatial_increment)),
                           np.linspace(y_min, y_max,
                                       np.ceil((y_max-y_min)/args.spatial_increment)),
                           np.linspace(z_min, z_max,
                                       np.ceil((z_max-z_min)/args.spatial_increment)))
        ...
        A_vacuum_theta = (args.bias_field_magnitude/2. *
                          np.sqrt(mesh_wo_edges[0]**2. +
                                  mesh_wo_edges[1]**2.))
        A_vacuum_x = A_vacuum_theta * np.cos(np.arctan2(mesh_wo_edges[1],
                                                        mesh_wo_edges[0]))
        A_vacuum_y = A_vacuum_theta * np.sin(np.arctan2(mesh_wo_edges[1],
                                                        mesh_wo_edges[0]))
        ...
        bx_derivative = remove_edges_derivative(bx_derivative)
        by_derivative = triangulate_derivatives(mesh, by_triangulation, by_interpolator,
                                                increment=args.derivative_increment)
        by_derivative = remove_edges_derivative(by_derivative)
        bz_derivative = triangulate_derivatives(mesh, bz_triangulation, bz_interpolator,
                                                increment=args.derivative_increment)
        bz_derivative = remove_edges_derivative(bz_derivative)


        current = current_on_mesh([bx_derivative,
                                   by_derivative,
                                   bz_derivative])
        #print 'mesh', mesh[0].shape
        #print 'mesh_wo_edges', mesh_wo_edges[0].shape
        ...
        b_field, b_field_norm = b_field_on_mesh([bx_interpolator,
                                                 ...
                                                 bz_interpolator],
                                                mesh_wo_edges,
                                                bias=args.bias_field_magnitude)
        ...
        density = boxcar_filter_quantity_mesh(density,
                                              args.filter_width)
        temperature = boxcar_filter_quantity_mesh(temperature,
                                                  args.filter_width)
        ...
        for direction in xrange(len(current)):
            current[direction] = boxcar_filter_quantity_mesh(current[direction],
                                                             args.filter_width)
        #print 'current_filtered', current[0].shape
        ...
        mach_x_plus_interpolator = mach_y_interpolators[u_x_time_points_plus[time_point]]
        mach_x_minus_interpolator = mach_y_interpolators[u_x_time_points_minus[time_point]]
        mach_y_interpolator = mach_y_interpolators[time_point]
        mach_z_interpolator = mach_z_interpolators[time_point]

        mach_x_plus = scalar_on_mesh(mach_x_plus_interpolator, mesh_wo_edges[:2])
        mach_x_minus = scalar_on_mesh(mach_x_minus_interpolator, mesh_wo_edges[:2])
        mach_y = scalar_on_mesh(mach_y_interpolator, mesh_wo_edges[:2])
        mach_z = scalar_on_mesh(mach_z_interpolator, mesh_wo_edges[:2])
        te = temperature

        u_i_x_plus = np.sqrt(te*q_e/m_i)*mach_x_plus
        u_i_x_minus = np.sqrt(te*q_e/m_i)*mach_x_minus
        u_i_y = np.sqrt(te*q_e/m_i)*mach_y
        u_i_z = np.sqrt(te*q_e/m_i)*mach_z

        u_i_x_plus = np.reshape(u_i_x_plus, mesh_wo_edges[0].shape)
        u_i_x_minus = np.reshape(u_i_x_minus, mesh_wo_edges[0].shape)
        u_i_y = np.reshape(u_i_y, mesh_wo_edges[0].shape)
        u_i_z = np.reshape(u_i_z, mesh_wo_edges[0].shape)

        ion_velocity_plus = [u_i_x_plus, u_i_y, u_i_z]
        ion_velocity_minus = [u_i_x_minus, u_i_y, u_i_z]

        grad_ion_velocity_x_plus = [np.gradient(ion_velocity_plus[0],
                                                args.spatial_increment,
                                                axis=1),
                                    np.gradient(ion_velocity_plus[0],
                                                args.spatial_increment,
                                                axis=0),
                                    np.gradient(ion_velocity_plus[0],
                                                args.spatial_increment,
                                                axis=2)]
        grad_ion_velocity_x_minus = [np.gradient(ion_velocity_minus[0],
                                                 args.spatial_increment,
                                                 axis=1),
                                     np.gradient(ion_velocity_minus[0],
                                                 args.spatial_increment,
                                                 axis=0),
                                     np.gradient(ion_velocity_minus[0],
                                                 args.spatial_increment,
                                                 axis=2)]
        grad_ion_velocity_y = [np.gradient(ion_velocity_plus[1],
                                           args.spatial_increment,
                                           axis=1),
                               np.gradient(ion_velocity_plus[1],
                                           args.spatial_increment,
                                           axis=0),
                               np.gradient(ion_velocity_plus[1],
                                           args.spatial_increment,
                                           axis=2)]
        grad_ion_velocity_z = [np.gradient(ion_velocity_plus[2],
                                           args.spatial_increment,
                                           axis=1),
                               np.gradient(ion_velocity_plus[2],
                                           args.spatial_increment,
                                           axis=0),
                               np.gradient(ion_velocity_plus[2],
                                           args.spatial_increment,
        ...
        ion_vorticity_plus =  current_on_mesh([grad_ion_velocity_x_plus,
                                               grad_ion_velocity_y,
                                               grad_ion_velocity_z], mu_0=1.)
        ion_vorticity_minus =  current_on_mesh([grad_ion_velocity_x_minus,
                                                grad_ion_velocity_y,
                                                grad_ion_velocity_z], mu_0=1.)

        ion_vorticity_plus_raw = np.array(ion_vorticity_plus)
        ion_vorticity_minus_raw = np.array(ion_vorticity_minus)

        for direction in xrange(len(ion_vorticity_plus)):
            ion_vorticity_plus[direction] = boxcar_filter_quantity_mesh(ion_vorticity_plus[direction],
                                                                        args.filter_width)
            ion_vorticity_minus[direction] = boxcar_filter_quantity_mesh(ion_vorticity_minus[direction],
                                                                         args.filter_width)
        ...
        vector_potential = devore_invert_curl(mesh_wo_edges,
                                              b_field)
        mesh = remove_edges_mesh([np.array(mesh_wo_edges[0]),
                                  np.array(mesh_wo_edges[1]),
                                  np.array(mesh_wo_edges[2])],
                                 x_start=3, x_end=-3,
                                 y_start=3, y_end=-3,
                                 z_start=2, z_end=-2)
        b_field_norm = remove_edges_vector([np.array(b_field[0]),
                                            np.array(b_field[1]),
                                            np.array(b_field[2])],
                                           x_start=3, x_end=-3,
                                           y_start=3, y_end=-3,
                                           z_start=2, z_end=-2)
        A_vacuum_x = remove_edges_scalar(np.array(A_vacuum_x),
                                         x_start=3, x_end=-3,
                                         y_start=3, y_end=-3,
                                         z_start=2, z_end=-2)
        A_vacuum_y = remove_edges_scalar(np.array(A_vacuum_y),
                                         x_start=3, x_end=-3,
                                         y_start=3, y_end=-3,
                                         z_start=2, z_end=-2)
        #print 'current before second cut', current[0].shape
        current = remove_edges_vector([np.array(current[0]),
                                       np.array(current[1]),
                                       np.array(current[2])],
                                      x_start=3, x_end=-3,
                                      y_start=3, y_end=-3,
                                      z_start=2, z_end=-2)
        #print 'current after second_cut', current[0].shape
        #print 'mesh after second cut', mesh[0].shape
        current_unfiltered = remove_edges_vector([np.array(current_unfiltered[0]),
                                                  np.array(current_unfiltered[1]),
                                                  np.array(current_unfiltered[2])],
                                                 x_start=3, x_end=-3,
                                                 y_start=3, y_end=-3,
                                                 z_start=2, z_end=-2)
        density = remove_edges_scalar(np.array(density),
                                      x_start=3, x_end=-3,
                                      y_start=3, y_end=-3,
                                      z_start=2, z_end=-2)
        temperature = remove_edges_scalar(np.array(temperature),
                                          x_start=3, x_end=-3,
                                          y_start=3, y_end=-3,
                                          z_start=2, z_end=-2)
        density_smoothed_plane_normalized = remove_edges_scalar(np.array(density_smoothed_plane_normalized),
                                                                x_start=3, x_end=-3,
                                                                y_start=3, y_end=-3,
                                                                z_start=2, z_end=-2)
        temperature_smoothed_plane_normalized = remove_edges_scalar(np.array(temperature_smoothed_plane_normalized),
                                                                x_start=3, x_end=-3,
                                                                y_start=3, y_end=-3,
                                                                z_start=2, z_end=-2)
        density_unfiltered = remove_edges_scalar(np.array(density_unfiltered),
                                                 x_start=3, x_end=-3,
                                                 y_start=3, y_end=-3,
                                                 z_start=2, z_end=-2)
        temperature_unfiltered = remove_edges_scalar(np.array(temperature_unfiltered),
                                                     x_start=3, x_end=-3,
                                                     y_start=3, y_end=-3,
                                                     z_start=2, z_end=-2)
        density_unfiltered_plane_normalized = remove_edges_scalar(np.array(density_unfiltered_plane_normalized),
                                                                  x_start=3, x_end=-3,
                                                                  y_start=3, y_end=-3,
                                                                  z_start=2, z_end=-2)

        temperature_unfiltered_plane_normalized = remove_edges_scalar(np.array(temperature_unfiltered_plane_normalized),
                                                                        x_start=3, x_end=-3,
                                                                        y_start=3, y_end=-3,
                                                                        z_start=2, z_end=-2)
        b_field = remove_edges_vector([np.array(b_field[0]),
                                       np.array(b_field[1]),
                                       np.array(b_field[2])],
                                      x_start=3, x_end=-3,
                                      y_start=3, y_end=-3,
                                      z_start=2, z_end=-2)
        ion_velocity = remove_edges_vector([np.array(ion_velocity_plus[0]),
                                            np.array(ion_velocity_plus[1]),
                                            np.array(ion_velocity_plus[2])],
                                           x_start=3, x_end=-3,
                                           y_start=3, y_end=-3,
                                           z_start=2, z_end=-2)
        ion_vorticity = remove_edges_vector([np.array(ion_vorticity_plus[0]),
                                             np.array(ion_vorticity_plus[1]),
                                             np.array(ion_vorticity_plus[2])],
                                            x_start=3, x_end=-3,
                                            y_start=3, y_end=-3,
                                            z_start=2, z_end=-2)
        ion_vorticity_raw = remove_edges_vector([np.array(ion_vorticity_plus_raw[0]),
                                             np.array(ion_vorticity_plus_raw[1]),
                                             np.array(ion_vorticity_plus_raw[2])],
                                            x_start=3, x_end=-3,
                                            y_start=3, y_end=-3,
                                            z_start=2, z_end=-2)
        vector_potential = remove_edges_vector([np.array(vector_potential[0]),
                                                np.array(vector_potential[1]),
                                                np.array(vector_potential[2])],
                                               x_start=3, x_end=-3,
                                               y_start=3, y_end=-3,
                                               z_start=2, z_end=-2)
        ...
        (ion_velocity_raw_ref,
         ion_vorticity_raw_ref) = ref_fields(mesh, ion_vorticity_raw)

        if args.task == 'write_to_vtk':
            fields = (list(b_field) + list(b_field_norm) +
                      [A_vacuum_x] + [A_vacuum_y] +
                      list(current) + list(current_unfiltered) +
                      [density] + [temperature] +
                      [density_smoothed_plane_normalized] +
                      [temperature_smoothed_plane_normalized] +
                      [density_unfiltered] + [temperature_unfiltered] +
                      [density_unfiltered_plane_normalized] +
                      [temperature_unfiltered_plane_normalized] +
                      list(ion_velocity) +
                      list(ion_vorticity) + list(ion_vorticity_raw) +
                      list(vector_potential) + list(b_field_ref) +
                      list(vector_potential_ref) + list(ion_velocity_ref) +
                      list(ion_vorticity_ref) + list(ion_velocity_raw_ref) +
                      list(ion_vorticity_raw_ref))
            ...
            write_fields_and_currents_to_structured_mesh(now,
                                                         args.output_prefix,
                                                         x, y, z, variables,
                                                         time_point)
        ...
        if args.task == 'calculate_helicity':

            dx, dy, dz = (args.spatial_increment,)*3
            ...
            with open(out_dir +
                      "/kinetic_helicity_raw_vorticity",
                      "a") as myfile:
...
def parse_args():
    ...
    parser.add_argument('--alpha',
                        help=("alpha parameter in ion velocity,"
                              "usually 1 can be set later by VisIt"),
    ...
    parser.add_argument('--output_prefix',
                        ...
                        default='Bdot_triple_probe_quantities')
    parser.add_argument('--bias_field_magnitude',
                        help='magnitude of axial bias magnetic field', default=0.02)
    parser.add_argument('--smooth_factor_alpha_z03_fit',
                        help='FITPACK smooth parameter alpha fit in 30cm plane',
                        default=8e10)
    parser.add_argument('--smooth_factor_alpha_z04_fit',
                        help=("FITPACK smooth parameter alpha"
                              "fit in 40cm plane"), default=7e10)
    parser.add_argument('--task',
                        help=("write_to_vtk, write_to_npz,"
                              "or calculate_helicity"),
                        default='write_to_vtk')
...
def fit_y_z_alphas(data_y, data_z, mesh, s=None):
    r"""
    """
    all_data = np.concatenate((data_y['a_out'][0],
                               data_z['a_out'][0]))
    all_points_x = np.concatenate((data_y['x_out'],
                                   data_z['x_out']))
    all_points_y = np.concatenate((data_y['y_out'],
                                   data_z['y_out']))
    splines = SmoothBivariateSpline(all_points_x, all_points_y, all_data, s=s)
    points_x = mesh[0][:, :, -1].ravel()
    points_y = mesh[1][:, :, -1].ravel()
    alpha_interp = np.reshape(splines(points_x, points_y, grid=False),
                              mesh[0][:, :, -1].shape)
    return alpha_interp
...
def fit_z_alphas(data_z, mesh, s=None):
    ...
    splines = SmoothBivariateSpline(all_points_x,
                                    all_points_y,
                                    all_data, s=s)
    points_x = mesh[0][:, :, -1].ravel()
    points_y = mesh[1][:, :, -1].ravel()
    alpha_interp = np.reshape(splines(points_x,
                                      points_y,
                                      grid=False),
                              mesh[0][:, :, -1].shape)
    return alpha_interp
...
def plot_spline_data_knots(scalar_resampled, x_grid, y_grid, data_dict, knots,
    ...
    ax.plot_wireframe(x_grid[::2,::2],
                      y_grid[::2,::2],
                      scalar_resampled[::2,::2],
                      colors=spline_color)
    ax.set_xlabel('x [m]')
    ax.set_ylabel('y [m]')
    plt.show()
...
def remove_edges_derivative(derivative,
                            x_start=2, x_end=None,
                            y_start=0, y_end=-2,
                            z_start=0, z_end=-1):
    r"""
    """
    for index in xrange(len(derivative)):
        derivative[index] = derivative[index][y_start:y_end, x_start:x_end, z_start:z_end]
    return derivative
...
def write_fields_and_currents_to_structured_mesh(date, visit_file_name,
    ...
    r"""
    """
    file_prefix = '../output/write_to_vtk/' + date + '/' + visit_file_name
    path = file_prefix + str(time_point).zfill(4)
    print path
...
def plot_planes(planes, mesh, quantity, interpolator=True,
    ...
    for i, plane in enumerate(planes):
        ...
        contour = axes[i].contourf(mesh[0][:, :, 0],
                                   mesh[1][:, :, 0], values)
